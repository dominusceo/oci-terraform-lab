resource "oci_core_instance" "test_instance" {
  count               = 2
  availability_domain = var.adDomain
  compartment_id      = var.compartment
  shape               = "VM.Standard.E2.1.Micro"
  display_name = "server-${count.index + 1}"

  source_details {
    #All images: https://docs.oracle.com/en-us/iaas/images/image/4d8a588e-bb9d-428d-af29-245ec57aa41a/
    source_id   = "ocid1.image.oc1.us-sanjose-1.aaaaaaaa3stdbyfdqhcdykpzf7ndzwudklnp77wo5lym7rp33ysspk6o2m4q"
    source_type = "image"
  }

  metadata = {
    ssh_authorized_keys = "ssh-rsa  terraform-ssh-key"
 }
 create_vnic_details {
    subnet_id = oci_core_subnet.priv_subnet1.id
 }
 connection {
    type        = "ssh"
    host        = "self.public_ip"
    #host       = self.test_instance[count.index].public_ip
    user        = "opc"
    timeout     = "3m"
    private_key = file("~/.ssh/tf-private.key")
  
 }

 provisioner "local-exec" {
    #command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ansible_user} -i '${self.public_ip}' --private-key ${var.ansible_ssh_private_key} setup.yaml"
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ansible_user} -i 'host.ini' --private-key ${var.ansible_ssh_private_key} setup.yaml"
  }
}
